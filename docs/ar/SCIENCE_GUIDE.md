...
الداخلي `CHIP` ("من файл config.yaml" كما تنص الحاشية في الصفحة).
...
مما يحافظ على ثباتية الموجة مع بقاء مظهرها "حياً" كصفحة تعانق الحواف. وتظهر التسمية
المقاسة بدقة المواصفة `±50 µs` / `±5 µs`.

### ٩-٢ الانحراف ("jitter") ومخطط العين

الانحراف هو التشتّت الإحصائي لحافة إشارة حول موضعها المثالي. **مخطط العين** يطابق
40 حافة صاعدة، كلٌّ منها مُزاح عشوائياً ضمن نطاق الانحراف؛ وعرض "الضباب" عند زاوية
